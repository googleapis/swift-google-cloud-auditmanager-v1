...
Add `swift-google-cloud-auditmanager-v1` as a package dependency:

```bash
swift package add-dependency https://github.com/googleapis/swift-google-cloud-auditmanager-v1.git --from 0.1.0-preview
```

Then add `GoogleCloudAuditManagerV1` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudAuditManagerV1 <target-name> --package swift-google-cloud-auditmanager-v1
```
